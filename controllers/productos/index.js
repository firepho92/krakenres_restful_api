...
module.exports = function (router) {
    ...
    router.get('/:id', function (req, res) {
        client.query('SELECT productos_id, nombre_producto FROM productos WHERE productos_id=$1', [req.params.id], function (error, results) {
            if (error) throw error;
    ...
    router.post('/by_name', function(req, res) {
        ...
        client.query('SELECT productos_id, nombre_producto FROM productos WHERE nombre_producto LIKE $1', [name], function(error, results) {
            if(error) throw error;
            res.send(results.rows)
        });
    })
    ...
    router.post('/', function (req, res) {
        ...
        connection.query('INSERT INTO horarios SET ?', horario, function(error, results) {
            if(error) throw error;
            res.send(results);
        });
    });
